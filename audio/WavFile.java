...
public class WavFile
{
	...
	public static WavFile openWavFile(File file) throws IOException, WavFileException
	{
		...
		wavFile.streamLength = file.length();
	...
	public static WavFile loadWavFile(InputStream iStream) throws IOException, WavFileException
	{
		// Instantiate new Wavfile and store the file reference
		WavFile wavFile = new WavFile();
		wavFile.file = null;
	...
	public static void loadWavFile(WavFile wavFile) throws IOException, WavFileException
	{
		...
		if (wavFile.streamLength > 0 && wavFile.streamLength != chunkSize+8) {
			throw new WavFileException("Header chunk size (" + chunkSize + ") does not match file size (" + wavFile.streamLength + ")");
		}
	...
	public int readFrames(float[] sampleBuffer, int offset, int numFramesToRead) throws IOException, WavFileException
	{
		...
		for (int f=0 ; f<numFramesToRead ; f++)
		{
			...
			for (int c=0 ; c<numChannels ; c++) 
				sampleBuffer[c*numChannels + offset] = (float)(floatOffset + (double) readSample() / floatScale);
